...
/***********************************************************************
** $Source: /home/cvs/nss/sonata-pkg/dx/prototype/tasks/retrieval.cc,v $
**
** Retrieval simulation task
**
** $Header: /home/cvs/nss/sonata-pkg/dx/prototype/tasks/retrieval.cc,v 1.1.1.1 2006/08/02 05:30:28 kes Exp $
** $Log: retrieval.cc,v $
...
RetrievalTask(void *arg)
{
	...
	retrievalInfo_t retrievalInfo;

	if (!arg)
		SysFatal(NSS_NA, function, "RetrievalTask");

	// get the initialization structure
	retrievalInfo = *((retrievalInfo_t *) arg);

	while (1) {
		// receive a storage message
		if (!(msg = QReceive(retrievalInfo.msgQ, &err, &tv)))
