...
short bitreverser(short fieldsize,short n)
    {short i,rev;
     rev = 0;
     for(i=0;fieldsize > 1;i++)
       {rev = rev << 1;
	rev = rev + (n % 2);
	n = n >> 1;
...
void unscramble(short rows,short cols,signed char *data)
    {short temp,i,j,ibitrev;
     for(i=0;i<rows;i++)
	 {ibitrev = bitreverser(rows,i);
	  if(i < ibitrev)
	      for(j=0;j<cols;j++)
		  {temp = data[i*cols+j];
...
void dadd_conquer(long rcount,long rlength,long rgap,long groupdrift,
	     char *darray)
     {char *lowrow,*highrow;
      short half,i,ibitrev,character;
      half = 1;
      if(rcount > 2)
	 {half = rcount/2;
	  dadd_conquer(half,rlength,rgap,groupdrift,darray);
	  dadd_conquer(half,rlength,rgap,groupdrift,rowadrs(half*rgap,rlength,
		  darray));
	  }

      for(i=0;i<half;i++)
	  {ibitrev = bitreverser(half,i);
	   lowrow = rowadrs(ibitrev*rgap,rlength,darray);
	   highrow = rowadrs((half+ibitrev)*rgap,rlength,darray);
	   topdowndaddpair(rlength,i+half*groupdrift,lowrow,highrow);
	  }
      }



void dadd_divide(long rowcount,long rowlength,char *darray)
    {long i,bitrv;
     if(rowcount > CACHEROWS)
	 {for(i=0;i<CACHEROWS;i++) //cut the block into subblocks and run dadd
				   //on each subblock     
	     dadd_divide(rowcount/CACHEROWS,rowlength,
		       rowadrs(i*rowcount/CACHEROWS,rowlength,darray));

	  for(i=0;i<rowcount/CACHEROWS;i++) // run dadd on the set of drift i vectors
	    {      bitrv = bitreverser(rowcount/CACHEROWS,i);
		     dadd_conquer(CACHEROWS,rowlength,rowcount/CACHEROWS,
		     bitrv,rowadrs(i,rowlength,darray));
	    }
	 }
       //rowcount is too small to divide, so just conquer
     else {//printf("conquer from divnconq  2\n");
	   dadd_conquer(rowcount,rowlength,1,0,darray);
	   }
    }
...
//    printarray(NROWS,NCOLS,test33);
    ftime(&starttime);
    dadd_divide(NROWS,NCOLS,test11);
    ftime(&endtime);
