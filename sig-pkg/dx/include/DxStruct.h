...
struct ObsData {
	...
	uint32_t nFrames;				// number of frames of actual obs data
...
//	uint32_t bandwidth;				// number of subchannels processed
	ip_src ipOption;
	op_src cwOutputOption;
	op_src cdOutputOption;
	op_src blOutputOption;
	op_src pulseOutputOption;
	uint32_t noisePowerLevel;		// for ipOption == sw_patgen
	uint32_t preBLSubchannel;		// subchannel for fake pre-baseline signal
	uint32_t postBLSubchannel;		// subchannel for fake post-baseline signal
	uint32_t postBLStartBin;		// start bin for fake post-baseline signal
	float32_t postBLSignalPower;	// peak amplitude for post-baseline signal
	uint32_t postBLOffTime;			// # subchannel spectra post-baseline
									//	signal is off
	uint32_t postBLOnTime;			// # subchannel spectra post-baseline
									//	signal is on
	float32_t postBLObsDriftBins;	// # of bins post-baseline signal drifts
									//	during obs
	OpsMask ops;					// operations mask
//	uint32_t obsFlags;				// see #defines below
//	std::bitset<SUBCHANNEL_MASK_LEN> subchannelMask;

	ObsData(): maxPulsesPerHalfFrame(0), maxPulsesPerSubchannelPerHalfFrame(0),
			centerFreq(0), cwResolution(RES_UNINIT), observationId(-1),
			nFrames(0), resetBaseline(true), baselineDelay(0),
			baselineWeighting(1), pulseThreshold(0), //bandwidth(0),
			ipOption(channelizer), cwOutputOption(normal),
			cdOutputOption(normal), blOutputOption(normal),
			pulseOutputOption(normal),
			noisePowerLevel(0), preBLSubchannel(0), postBLSubchannel(0),
			postBLStartBin(0), postBLSignalPower(0), postBLOffTime(0),
			postBLOnTime(0), postBLObsDriftBins(0) {}
};
