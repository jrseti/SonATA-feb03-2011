...

{
	...
	// set up the post-BL wave table
	signal.init(state->getSamplesPerSubchannelFrame());
...
Spectrometer::setup(Activity *act)
{
	...
	// create the simulated signal array
	signal.init(state->getSamplesPerFrame());
...
//	const ResData *res = activity->getResData();
		...
		int32_t hf)
{
	...
	// inject a 256Hz signal in a single subchannel in the sw pattern case
	if ((subchannel == (int32_t) obs.preBLSubchannel)
			&& (obs.ipOption == sw_pattern)) {
		// get the address of the half frame data
		ComplexFloat32 rPower(4.0, 0.0);
		ComplexFloat32 iPower(0.0, 4.0);
		for (int32_t i = 0; i < channel->getSamplesPerSubchannelHalfFrame();
				i += 4) {
			hfData[i] += iPower;
			hfData[i+1] += rPower;
			hfData[i+2] -= iPower;
	...
	// inject a signal in the frame based on configuration values specified
	// for the observation.
	int32_t samples = state->getSamplesPerHalfFrame();
	if ((subchannel == (int32_t) obs.postBLSubchannel)
				&& (obs.ipOption == sw_pattern)) {
		for (int32_t i = 0; i < samples; ++i) {
			signal.bin += signal.drift;
			signal.index += signal.bin;
			if (signal.index > samples)
				signal.index -= samples;
			if (signal.dutyCycle < obs.postBLOnTime) {
				int32_t j = (int32_t) (signal.index + 0.5);
				hfData[i].real() += obs.postBLSignalPower
						* signal.wave[j+samples/4];
				hfData[i].imag() += obs.postBLSignalPower
						* signal.wave[j%samples];
			}
			if (++signal.dutyCycle == obs.postBLOnTime + obs.postBLOffTime)
				signal.dutyCycle = 0;
		}
	}
