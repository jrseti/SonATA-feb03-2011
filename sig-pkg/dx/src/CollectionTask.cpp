...
CollectionTask::setStartObs()
{
	...
	obs.nFrames = activity->getFrames();
...
//	obs.bandwidth = params.assignedBandwidth;
	...
	if (cmdArgs->swapInputs() || operations.test(FREQ_INVERSION))
		obs.ops.set(INVERT_FREQ);
	if (cmdArgs->noSSE())
		obs.baselineDelay = cmdArgs->getBaselineDelay();
	if (cmdArgs->simulateInput())
		obs.ipOption = sw_pattern;

	if (cmdArgs->noSSE() || params.baselineInitAccumHalfFrames > 0)
		obs.resetBaseline = 1;
	else
		obs.resetBaseline = 0;
	...
	// set up test signal insertion
	obs.noisePowerLevel = cmdArgs->getSimulationNoise();
	obs.preBLSubchannel = cmdArgs->getPreBLSubchannel();
	obs.postBLSubchannel = cmdArgs->getPostBLSubchannel();
	obs.postBLStartBin = cmdArgs->getPostBLStartBin();
	obs.postBLSignalPower = cmdArgs->getPostBLPower();
	obs.postBLOffTime = cmdArgs->getOffHalfFrames()
			* channel->getSamplesPerSubchannelHalfFrame();
	obs.postBLOnTime = cmdArgs->getOnHalfFrames()
			* channel->getSamplesPerSubchannelHalfFrame();
	obs.postBLObsDriftBins = cmdArgs->getPostBLDriftRate() *
			obs.nFrames;
