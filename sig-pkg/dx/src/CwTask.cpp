...
CwTask::extractArgs()
{
	...
	Assert(detectionQ);
#ifdef notdef
	superClusterer = cwArgs->superClusterer;
	Assert(superClusterer);

	// create clustering objects, which reference the superClusterer
...
//	rightHit = new ClusterHit(rightClusterer);
//	leftHit = new ClusterHit(leftClusterer);
#endif
...
CwTask::unpack(DaddSlope slope, Polarization pol)
{
	...
	// if it's been requested, pack the buffer with a very simple
	// pattern
	if (cmdArgs->insertPath())
		insertPath(pol);
...
#ifdef notdef
void
CwTask::reportHit(const DaddPath& path)
{
//	clusterer->recordHit(path.bin, path.drift, path.power);
	...
	case POL_RIGHTCIRCULAR:
	case POL_XLINEAR:
		rightClusterer->recordHit(path.bin, path.drift, path.power);
		break;
	case POL_LEFTCIRCULAR:
	case POL_YLINEAR:
		leftClusterer->recordHit(path.bin, path.drift, path.power);
		break;
	default:
		Assert(0);
	}
...
//
// insertPath: insert a path into the buffer
//
// Description:
//		Inserts a path into the packed slice buffer with the specified
//		starting bin, drift and power.
// Notes:
//		By putting the data into the packed slice buffer, we can also
//		test the unpack logic as well as the DADD pathsum logic.
//
void
CwTask::insertPath(Polarization pol)
{
#ifdef notdef
	::uint8_t binVal;
	int32_t binsPerIndex, bin, shift;
	int32_t idx, stride;
	uint32_t val, *pData0, *pData1;
	float32_t driftRate;

	// get the path insertion parameters
	bin = cmdArgs->getInsertBin();
	driftRate = cmdArgs->getDriftRate();
	binVal = cmdArgs->getBinVal();

	binsPerIndex = (CWD_BINS_PER_BYTE * sizeof(uint32_t));
	stride = spectrumBins / binsPerIndex;

	// drift requires that the bin be recomputed for
	// each spectrum
	for (int i = 0; i < spectra; i++) {
		bin = (int32_t) (cmdArgs->getInsertBin() + i * driftRate);
		idx = bin / binsPerIndex;
		bin = bin % binsPerIndex;
		shift = ((binsPerIndex - (bin + 1)) % binsPerIndex) * CWD_BITS_PER_BIN;
		// create the packed data value
		val = binVal << shift;
		pData0[idx] = val;
		pData1[idx] = val;
		pData0 += stride;
