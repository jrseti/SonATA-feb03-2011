...
//	float32_t *baseline;
//	float32_t *newBaseline;

	// Structures used to measure spectrometer timing
    struct timeval tvE;
    struct timeval tvL;
    struct timezone tz;
	...
	struct SimSignal {
		...
		SimSignal(): initialized(false), wave(0) {}
		~SimSignal() { fftwf_free(wave); }
		void init(int32_t samples_) {
			initialized = false;
			// initialize the simulated signal array
			if (wave)
				fftwf_free(wave);
			wave = static_cast<float32_t *>
					(fftwf_malloc(samples_ * sizeof(float32_t)));
			Assert(wave);
			float32_t w, twoPi = M_PI * 2;
			for (int32_t i = 0; i < samples_; ++i) {
				w = (i + 0.5) / samples_ * twoPi;
				wave[i] = sinf(w);
			}
			initialized = true;
		}
