...
SseOutputTask::handleMsg(Msg *msg)
{
	...
	if (Args::getInstance()->noSSE()) {
		// In this mode, there's no SSE to send the message to,
		// so we just toss 'em.
		//
		// Except, if it's the message that means the observation is
		// done, hang around for a while and then exit with a good
		// return code.
		LogWarning(ERR_NE, 0, "message %d for SSE ", hdr.code);
		if (hdr.code == DX_ACTIVITY_COMPLETE ||
			hdr.code == DONE_SENDING_CW_COHERENT_SIGNALS ||
			hdr.code == BEGIN_SENDING_CW_COHERENT_SIGNALS) {
			Timer timer;
			timer.sleep(3000);
			::exit(0);
		}
